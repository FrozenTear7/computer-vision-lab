...
x = tf.compat.v1.placeholder("float", [None, 64,64,3])
...
loss_triplets = tf.reduce_sum(input_tensor=tf.maximum(float(0), 1 - (tf.reduce_sum(input_tensor=tf.square(diff_neg), axis=-1) / (tf.reduce_sum(input_tensor=tf.square(diff_pos), axis=-1) + 0.01))))
loss_pairs = tf.reduce_sum(input_tensor=tf.reduce_sum(input_tensor=tf.square(diff_pos), axis=-1))
...
with tf.compat.v1.Session() as sess:
    ...
    tf.compat.v1.train.Saver().save(sess, "./model/model.ckpt")

...
categories = {"ape": 1, "benchvise": 2, "cam": 3, "cat": 4, "duck": 5}
    ...
    while True:
        ...
        for idx in range(batch_size):
            ...
            if pusher_is_same_category:  # same category different pose
                ...
                while (
                    pusher_category == category
                ):  # ensure pusher_category not same as anchor
                    pusher_category = np.random.randint(5)
...
def conv_net(x):
    ...
    pool1 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=2, padding="valid")

    # Second Convolutional layer + ReLU + MaxPool
    # conv2 = tf.layers.conv2d(
    #     inputs=pool1(conv1(x)),
    #     filters=7,
    #     kernel_size=[5, 5],
    #     padding="VALID",
    #     use_bias=True,
    ...
    #     inputs=conv2, pool_size=[2, 2], strides=2, padding="VALID"
    # )

    pool2 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=2, padding="valid")

    # Flatten
    fc1 = tf.reshape(pool2(conv2(pool1(conv1(x)))), [-1, 12 * 12 * 7])

    # First Fully Connected layer + ReLU
    fc1 = tf.layers.dense(inputs=fc1, units=256, activation=tf.nn.relu, use_bias=True)

    # Second Fully Connected layer
    fc2 = tf.layers.dense(inputs=fc1, units=16, use_bias=True)
    return fc2
...
def measure_model_acc(template_predictions, test_predictions):
    ...
    nearest_neighbor = NearestNeighbors(n_neighbors=1, algorithm="auto").fit(
...
x = tf.Variable(tf.ones(shape=[None, 64, 64, 3]), dtype=tf.float32)
...
loss_triplets = tf.reduce_sum(
    tf.maximum(
        float(0),
        1
        - (
            tf.reduce_sum(tf.square(diff_neg), -1)
            / (tf.reduce_sum(tf.square(diff_pos), -1) + 0.01)
        ),
    )
)
    ...
    for i in range(num_epoch):  # For each epoch
        for batch_i in range(num_iters):  # For each batch
            ...
            if iter_cnt % 300 == 0:
                ...
                (test_acc, angles, c_test, c_pred) = measure_model_acc(
                    pred_template, pred_test
                )
                y_test = c_test
                y_pred = c_pred
...
colors = list(
    map(lambda idx: pallette[idx // test_set.shape[1]], range(len(pred_test)))
)
